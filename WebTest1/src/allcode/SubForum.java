...
public class SubForum implements Serializable{
	...
	public Map <Integer, Post> get_rootPosts()
	{
		return this._rootPosts;
	}
	
	public List<Post> getRootPostsArrayList()
	{
		List<Post> rootPosts = new ArrayList<Post>();
		Collection<Post> cl = _rootPosts.values();
		Object tempposts[] = cl.toArray();
		for (int i = 0; i < tempposts.length; i++)
			rootPosts.add((Post)tempposts[i]);
		return rootPosts;
	}
